...
class WorkoutPlanDataTable extends BaseDataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ...
            ->addColumn('duration', function ($plan) {
                return $plan->duration_weeks ? $plan->duration_weeks . ' weeks' : '-';
            })
            ->addColumn('action', function ($plan) {
                $showUrl = route('admin.workout-plans.show', $plan->id);
                $editUrl = route('admin.workout-plans.edit', $plan->id);
                $deleteUrl = route('admin.workout-plans.destroy', $plan->id);
                ...
                if (auth()->user()->can('edit workout plans') && 
                ...
                if (auth()->user()->can('delete workout plans') && 
                    (!auth()->user()->hasRole('trainer') || $plan->trainer_id === auth()->id())) {
                    $html .= '<form action="' . $deleteUrl . '" method="POST" class="inline" data-confirm="true" data-confirm-title="Delete Workout Plan" data-confirm-message="Deleting ' . e($plan->plan_name) . ' will remove it from the member history. Continue?" data-confirm-button="Delete Plan" data-confirm-tone="danger">';
    ...
    protected function getColumns(): array
    {
        ...
        $columns = array_merge($columns, [
            ...
            Column::make('duration')->title('Duration')->width('10%')->orderable(false)->searchable(false)->addClass('text-right'),
